...
{ 
  ...
  var FetchedContestNames = {};

  // for unread Tag
  var localStorageData = localStorage.getItem("FetchedContestURLs");
  //for first time run prev fetched urls should be equal to null 
  ...
  $.each(json.result.ongoing , function(i,post){ 
    ...
    if(localStorage.getItem(post.Platform)=="true" && flag==0){
      ...
      //no to contest with same name should come up again
      if(!(post.Name in FetchedContestNames)){
      FetchedContestNames[post.Name] = 1;
